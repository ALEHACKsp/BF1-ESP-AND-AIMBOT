...
void CGui::GUI_Init(HWND hWnd, IDirect3DDevice9* pDevice)
{
	//ImGui::CreateContext();
	...
	font = io.Fonts->AddFontFromFileTTF(pTahoma.c_str(), 14.0f);
	tabfont = io.Fonts->AddFontFromFileTTF(pTahoma.c_str(), 62.0f);
	style.Alpha = 0.0f;
...
void CGui::GUI_Begin_Render()
{
	...
	ImGui::SetWindowSize(ImVec2(io.DisplaySize.x, io.DisplaySize.y), ImGuiSetCond_Always);
...
{
	ImGui::GetIO().MouseDrawCursor = Globals::Opened;
	g_pGui->GUI_Begin_Render();
	if (Globals::Opened)
	{
		Render::DrawMenu();
	}
	if (g_Options.ESP.EnableESP) {
		Render::DrawVisuals();
	}
	if (g_Options.AIMBOT.aimbot) {
		Render::Runaimbot();
	}
	if (g_Options.MISC.RADAR.radarwindow)
	{
		Render::DrawRadars();
	}
	Render::Crosshair();
